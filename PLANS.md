...
- Graphical-device groundwork now includes host-side framebuffer geometry, packed pixel-format metadata, dirty-region tracking, immutable framebuffer snapshots, host ARGB conversion, a Swing framebuffer presentation backend, and an optional `RiscVContext`/syscall-layer framebuffer hook. Guest `/dev/fb0`, framebuffer ioctls, mmap integration, and CLI window creation are not wired yet.
