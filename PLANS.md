...
- The runtime includes Docker-like bind/tar mounts, virtual `/proc`, minimal virtual `/sys`, Linux-like `/dev`, minimal `NETLINK_ROUTE` interface enumeration, guest credentials, deterministic time, process/thread state, `clone`/`wait4`, terminal handling, and Gradle-managed example/test tasks.
