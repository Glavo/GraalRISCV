...
- Linux runtime support includes process/thread state, mutable guest credentials, setuid/setgid exec credential transitions, child-exit `SIGCHLD` delivery, deterministic RT signal wait/queue validation, deterministic and guest-adjustable realtime clocks, tracked `adjtimex`/`clock_adjtime` and `getitimer`/`setitimer` timer state, POSIX `timer_create`/`timer_settime`/`timer_gettime`/`timer_delete` state, `clone`/`wait4`/`waitid`, Linux pidfd open/signal/descriptor-duplication support, current-process `kcmp` resource comparison, classic and split futex wait/wake operations, resource-limit reporting and lowering through `getrlimit`/`setrlimit`/`prlimit64`, single-CPU scheduling policy/priority, deadline `sched_attr`, I/O priority, affinity syscalls, process personality state, and root-controlled UTS nodename/domainname state, positioned scalar/vector file I/O, modern file and exec entry points (`openat2`, `memfd_create`, `execveat`), file-range copying, `sendfile`/`tee`/`vmsplice` and `process_vm_readv`/`process_vm_writev` memory transfers, file preallocation, range sync, readahead validation, whole-file advisory `flock`, validated memory-sync and memory-locking no-op syscalls, explicit Linux errors for unmodeled kernel administration, namespace, mount, security, filesystem-handle, AIO, IPC, keyring, and NUMA policy probes, descriptor close-on-exec and `close_range` handling, blocking `eventfd2` counters, Linux `timerfd_create`/`timerfd_settime`/`timerfd_gettime`, in-memory `inotify` descriptors with watch lifecycle support, Linux `signalfd4` descriptors for blocked child-exit `SIGCHLD` notifications, event-aware `epoll_pwait`/`epoll_pwait2`/`pselect6`/`ppoll` waits for guest-internal descriptors, Linux `#!` binary-format rewriting, terminal raw-mode behavior, and CLI environment overrides.
...
- Filesystem metadata and mutation include guest-visible uid, gid, chmod mode bits, process `umask`, Linux-style DAC, setgid directory inheritance, sticky-directory mutation checks, ancestor-directory search checks, deterministic empty extended-attribute reporting, regular node creation, and symbolic/hard link creation on writable host and tmpfs-style mounts.
