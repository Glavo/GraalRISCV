...
- Add syscall implementations as broader static Linux workloads require beyond the current `clone` parent-return compatibility, `faccessat`/`faccessat2`, `fcntl`, `futex` single-thread compatibility, `getcpu`, `getcwd`, `getppid`, `getrusage`, `gettimeofday`, `madvise`, `mprotect`, `nanosleep`, `newfstatat`, `prctl`, `prlimit64`, `readlinkat`, `riscv_hwprobe`, `sched_getaffinity`, `sched_yield`, `sigaltstack`, `times`, `uname`, and user/group identity syscall baseline.
