...
- Continue improving paged-memory slow paths, especially cross-page accesses and richer fault reporting; per-thread `Memory.Access`, split read/write access-local Unsafe-coordinate page caching, direct-mapped software TLB hits, zero-fill page caching, window-sized VarHandle radix page-table lookup, and VMA access protections are now in place.
