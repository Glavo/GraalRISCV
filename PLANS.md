...
- Continue profiling now that Gradle uses a project-local Polyglot resource cache, scalar initial-memory accesses delegate alignment and range checks to `MemorySegment`, scalar sparse mapped-memory accesses reuse cached mapped regions without per-access `MemoryAccess` allocation, initial memory range checks reuse a cached end address, decoded instructions are split into operation-group node subclasses, selected hot integer/control/load/store operations have direct instruction nodes, common integer compare and word-shift operations have direct instruction nodes, the root frame stages all 32 integer registers in static Truffle frame slots for migrated direct and operation-group nodes with branch-free frame reads, frame-backed `ecall` synchronizes only Linux syscall argument and result registers, decoded direct straight-line nodes no longer materialize sequential `pc` inside blocks, frame-backed block execution returns local `pc` without writing `MachineState.pc` at each block boundary, decoded direct terminators return their target `pc` to the enclosing block without reading it back from `MachineState`, the root dispatch loop carries block-to-block `pc` in a local variable, the root guest loop is isolated from setup/finally code with a local two-entry MRU block cache, decoded block-cache misses combine lookup and insertion and transfer back to the interpreter before decoding, default no-trace/no-limit block execution retires instruction counts in batches through a precomputed state flag, store side-effect checks are skipped before calling the side-effect handler when the image has no `tohost`, the decoded block cache no longer allocates boxed `Long` keys, the negative `BlockDispatchNode` inline-cache path has been removed, block execution no longer rechecks instruction terminators, instruction execution reuses a precomputed fall-through PC, decoded instruction register accesses skip public index validation, `MachineState` integer register reads rely on the backing array for `x0` and bounds behavior, and normal root cleanup no longer spills discarded frame registers on program exit.
...
- Investigate the remaining dispatch overhead, especially primitive block-cache lookup, remaining operation-group instruction body shape, and whether the `BlockNode` loop still costs enough to warrant a different compiled-block layout.
