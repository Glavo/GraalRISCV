...
- Complete the remaining RVV 1.0 instruction surface beyond the current foundation: widening/narrowing arithmetic, fixed-point rounding/saturation, remaining vector load/store forms such as segment and whole-register transfers, fault-only-first loads, floating-point reductions/fused operations/conversions, integer/vector scalar move corner cases, and precise illegal-instruction boundaries.
